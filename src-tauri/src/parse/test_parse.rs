...
fn test_simple_sentences() {
    ...
    assert!(
        ...
        "should have tts_para_0: {output}"
    );
    assert!(
        has_class(&output, "tts_para_1"),
        "should have tts_para_1: {output}"
    );
    assert!(
        output.contains("Call me Ishmael."),
        "should contain first sentence: {output}"
    );
    assert!(
        output.contains("Some years ago I thought I would sail."),
        "should contain second sentence: {output}"
    );
...
fn test_inline_elements_preserved() {
    ...
    assert!(
        output.contains("<em>"),
        "should preserve em element: {output}"
    );
...
fn test_multiple_inline_elements() {
    ...
    assert!(
        ...
        "should preserve strong element: {output}"
    );
...
fn test_abbreviations_not_split() {
    ...
    assert!(
        ...
        "should not split at abbreviations: {output}"
    );
    assert!(
        output.contains("Then he left."),
        "should have second sentence: {output}"
    );
...
fn test_skip_tags_removed() {
    ...
    assert!(
        ...
        "script should be removed: {output}"
    );
    assert!(
...
fn test_code_block_with_newlines() {
    ...
    assert!(
        ...
        "should have tts_code_block: {output}"
    );
    assert!(
        has_class(&output, "tts_para_0"),
        "should have tts_para_0: {output}"
...
fn test_code_inline_no_element_children_flattened() {
    ...
    assert!(
        !has_class(&output, "tts_code_block"),
        "should NOT have tts_code_block for inline code without element children: {output}"
    );
    assert!(
        output.contains("let x = 1;"),
        "should preserve code content: {output}"
    );
    assert_eq!(
        output,
...
fn test_anchor_url_resolution() {
    ...
    assert!(
        ...
        "should resolve relative URL: {output}"
    );
    assert!(
        has_class(&output, "tts_anchor"),
        "should have tts_anchor class: {output}"
    );
...
fn test_list_items() {
    ...
    assert!(
        ...
        "should have tts_para classes: {output}"
    );
...
fn test_code_block_no_newlines_no_element_children() {
    ...
    assert!(
        has_class(&output, "tts_para_0"),
        "pre should get tts_para_0: {output}"
    );
...
fn test_multiple_block_elements() {
    ...
    assert!(
        ...
        "should contain first paragraph: {output}"
    );
    assert!(
        output.contains("Another paragraph."),
        "should contain another paragraph: {output}"
    );
...
fn test_mixed_content_with_divs() {
    ...
    assert!(
        ...
        "should contain paragraph one: {output}"
    );
    assert!(
        output.contains("Paragraph two."),
        "should contain paragraph two: {output}"
    );
...
fn test_preserves_html_structure() {
    ...
    assert!(
        ...
        "should preserve article: {output}"
    );
...
fn test_fragment_only_url() {
    ...
    assert!(
        ...
        "should keep fragment-only URL: {output}"
    );
    assert!(
        !has_class(&output, "tts_anchor"),
        "fragment-only should not have tts_anchor: {output}"
    );
    assert_eq!(
        output,
...
fn test_bare_text_mixed_with_block_children() {
    ...
    assert!(
        ...
        "should contain bare text first sentence: {output}"
    );
    assert!(
        output.contains("Second sentence."),
        "should contain bare text second sentence: {output}"
    );
    assert!(
        output.contains("Block paragraph."),
        "should contain block paragraph: {output}"
    );
...
fn test_bare_text_before_blocks() {
    ...
    assert!(
        ...
        "should contain loose text before blocks: {output}"
    );
    assert!(
        output.contains("Title."),
        "should contain title: {output}"
    ...
    assert!(
        ...
        "should contain paragraph: {output}"
    );
...
fn test_single_bare_sentence_gets_tts_para() {
    ...
    assert!(
        ...
        "should contain first bare sentence: {output}"
    );
    assert!(
        output.contains("Second sentence here."),
        "should contain second bare sentence: {output}"
    );
...
fn test_code_block_preserves_tabs_and_empty_lines() {
    ...
    assert!(
        output.contains("\tlet x = 1;"),
        "should preserve tab indentation: {output}"
    );
    assert!(
        output.contains("\tlet y = 2;"),
        "should preserve tab on second let: {output}"
    );
...
fn test_pre_with_language_class() {
    ...
    assert!(
        ...
        "should preserve language-scala class: {output}"
    );
    assert!(
        output.contains("tabindex"),
        "should preserve tabindex: {output}"
    );
...
fn test_nonstandard_wrapper_with_block_child() {
    ...
    assert!(
        ...
        "should contain second paragraph: {output}"
    );
    assert!(
        has_class(&output, "tts_para_0"),
        "should have tts_para_0: {output}"
...
fn test_code_with_strong_inline_is_inline() {
    ...
    assert!(
        !has_class(&output, "tts_code_block"),
        "should NOT have tts_code_block: {output}"
    );
    assert!(
        has_class(&output, "tts_para_0"),
        "should have tts_para_0: {output}"
