...
<template>
  <div class="container">
    <article ref="divRef" class="page reader_view overflow-auto">
      <h1>{{ article.title }}</h1>
      <div v-html="article.body"></div>
    </article>

    <LinkPopup :url="externalUrl" @close="closeLinkPopup" />

    <dialog :open="deleteModal">
      <article>
        <h2><strong class="ti ti-trash-x">&#xf784;: {{ article.title }}</strong></h2>
        <footer>
          <button class="secondary" @click="toggleDeleteModal"><i class="ti ti-x">&#xeb55;</i></button>
          <button @click="deleteArticle"><i class="ti ti-check">&#xea5e;</i></button>
        </footer>
      </article>
    </dialog>
    ...
    <aside style="position: sticky; bottom: var(--safe-area-inset-bottom, 0);">
      <nav>
        <SpeakBar :divRef="divRef!" />
        <div role="group">
          <HomeButton />
          <button @click="openUrl(article.url)"><i class="ti ti-world-www">&#xf38f;</i></button>
          <button class="secondary" @click="toggleDeleteModal"><i class="ti ti-trash-x">&#xf784;</i></button>
        </div>
      </nav>
    </aside>
  </div>
