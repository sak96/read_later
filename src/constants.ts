export const SCROLL_THRESHOLD = 100

export const SEARCH_DEBOUNCE_MS = 500

export const ALERT_DURATION_MS = 5000

export const MIN_SEARCH_LENGTH = 3

// Settings
export const TUTORIAL_SHOWN = 'tutorial_speak_bar_shown'

export const EXPERIMENTAL_FETCHER = 'iframe_fetcher'

export const TTS_ENABLED = 'tts'

export const THEME = 'theme'

export const LOCALE = 'locale'

export const RATE = 'rate'

export const FONT_SCALE = 'fontScale'

// Webdav settings
export const WEBDAV_ENABLED = 'webdavEnabled'
export const WEBDAV_URL = 'webdavUrl'
export const WEBDAV_USERNAME = 'webdavUsername'
export const WEBDAV_PASSWORD = 'webdavPassword'
export const WEBDAV_PATH = 'webdavPath'
export const LAST_SYNCED_AT = 'lastSyncedAt'
export const WEBDAV_AUTH_TYPE = 'webdavAuthType'
